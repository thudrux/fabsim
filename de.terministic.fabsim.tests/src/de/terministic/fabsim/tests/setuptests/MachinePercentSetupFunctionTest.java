...
/*		model = new FabModel(new BasicRouting());	
		sink = (Sink)model.getSimComponentFactory().createSink();
		
		toolGroup=(ToolGroup) model.getSimComponentFactory().createToolGroup("Machine1", 2, ProcessingType.LOT);
		...
		model.getSimComponentFactory().createMinNumberOfToolsSetupChangeConditionAndAddToSetupStrategy(s1, 1, strategy);
		model.getSimComponentFactory().createMinNumberOfToolsSetupChangeConditionAndAddToSetupStrategy(s2, 1, strategy);
		...
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step1", toolGroup, 5L, s1, recipe);
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step2", sink, 0L, recipe);

		Recipe recipe2 = model.getSimComponentFactory().createRecipe("Recipe2");
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step1", toolGroup, 2L, s2, recipe2);
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step2", sink, 0L, recipe2);
		
		Recipe recipe3 = model.getSimComponentFactory().createRecipe("Recipe3");
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step1", toolGroup, 5L, s1, recipe3);
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step2", sink, 0L, recipe3);
		
		source = (LotSource)model.getSimComponentFactory().createSource("Source1", recipe, 1L);
		source.setLotSize(1);
		source.setAllowSplit(false);
		source2 = (LotSource)model.getSimComponentFactory().createSource("Source2", recipe2, 1L);
		source2.setLotSize(1);
		source2.setAllowSplit(false);
		source3 = (LotSource)model.getSimComponentFactory().createSource("Source3", recipe3, 2L);
		source3.setLotSize(1);
		source3.setAllowSplit(false);
