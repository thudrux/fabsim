...
SimulationEngine engine;
	...
	public void setUp() throws Exception {
		model = new FabModel();
		AbstractToolGroup toolGroup =model.getSimComponentFactory().createToolGroup("ToolGroup",1, ProcessingType.LOT);
		SetupState s1 = model.getSimComponentFactory().createSetupStateAndAddToToolGroup("State1", (ToolGroup)toolGroup);
		SetupState s2 = model.getSimComponentFactory().createSetupStateAndAddToToolGroup("State2", (ToolGroup)toolGroup);
		model.getSimComponentFactory().createSetupChangeAndAddToToolGroup(s1, s2, 1L, (AbstractHomogeneousResourceGroup) toolGroup);
		AbstractSetupStrategy strategy = model.getSimComponentFactory().createBasicConditionBasedSetupStrategyAndAddToToolGroup((AbstractHomogeneousResourceGroup) toolGroup);
		...
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step1", toolGroup, 1L, s1, ProcessType.LOT, recipe);
		...
		model.getSimComponentFactory().createProcessStepAndAddToRecipe("Step1", toolGroup, 1L, s2, ProcessType.LOT, recipe2);
